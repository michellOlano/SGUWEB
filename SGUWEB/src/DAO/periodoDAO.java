...
public class periodoDAO extends Conexion implements Serializable {
	...
	public String insertar(periodoC item) {	     		
        Connection c ;
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("{CALL DI.SP_MANTENIMIENTO_PERIODO(?,?,?,?,?,?,?,?)}");
            cs.setInt("ITEMWEB", 1);
            cs.setInt("INSTITUCION", item.getInstitucion());            
            cs.setString("PERIODO", item.getPeriodo());
            cs.setString("DESCRIPCION", item.getDesPeriodo());
            cs.setBoolean("ORDINARIO", item.isOrdinario());
            cs.setString("PERIODOANTERIOR", item.getPeriodoAnt().isEmpty()?null:item.getPeriodoAnt());
            cs.setString("PERIODOSIGUIENTE", item.getPeriodoSig().isEmpty()?null:item.getPeriodoSig());            
	...
	public String eliminar(periodoC item) {
        ...
        try {
            ...
            cs.setString("DESCRIPCION", item.getDesPeriodo());
            cs.setBoolean("ORDINARIO", item.isOrdinario());
            cs.setString("PERIODOANTERIOR", item.getPeriodoAnt());
            cs.setString("PERIODOSIGUIENTE", item.getPeriodoSig());            
            cs.setInt("ESTADOREGISTRO", item.getEstadoRegistro());
    ...
    public List<periodoC> mostrarPeriodoInstitucion(int institucion) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                item.setDesPeriodo(rs.getString("DESCRIPCION"));
                item.setPeriodoAnt(rs.getString("PERIODO_ANTERIOR"));
                item.setPeriodoSig(rs.getString("PERIODO_SIGUIENTE"));
                item.setOrdinario(rs.getBoolean("ORDINARIO"));
    ...
    public List<periodoC> getPeriodoAdminitrativo(int institucion) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                periodo.setDesPeriodo(rs.getString("DESCRIPCION"));
                periodo.setPeriodoAnt(rs.getString("PERIODO_ANTERIOR"));
                periodo.setOrdinario(rs.getBoolean("ORDINARIO"));
...
public List<periodoC> listarPeriodoPersonal(int institucion,String personal) {
