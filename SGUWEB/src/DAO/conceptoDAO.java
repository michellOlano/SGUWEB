...
public class conceptoDAO extends Conexion {
	...
	public String eliminar(conceptoC item) {
     ...
     public List<conceptoC> listaConcepto(int institucion,String periodo) {        
         Connection c ;
         CallableStatement cs ;
         ResultSet rs ;
         conceptoC item ;
         ...
         try {
             ...
             c = ConexionWeb();
             cs = c.prepareCall("SELECT B.*FROM  SIGU.TES_CONCEPTO_INSTITUCION_PERIODO A ,SIGU.TES_CONCEPTOS B WHERE A.INSTITUCION=? AND A.PERIODO LIKE ? AND B.CONCEPTO=A.CONCEPTO");
