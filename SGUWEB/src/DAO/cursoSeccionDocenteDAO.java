...
public class cursoSeccionDocenteDAO extends Conexiones.Conexion implements Serializable {
	...
	public List<cursoSeccionDocenteC> mostrarCursoSeccionDocente(int institucion, String periodo, String personal) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT A.*,B.DESCRIPCION DESCARRERA,B.ABREVIATURA ABRECARRERA,C.DESCRIPCION DESCURSO,D.DESCRIPCION DESSECCION FROM SIGU.HOR_CURSO_SECCION_DOCENTE A,UPA.ACA_CARRERAS B,UPA.ACA_CURSOS C,SIGU.HOR_SECCION D WHERE A.INSTITUCION=? AND A.PERIODO=? AND  A.PERSONAL like ? AND A.ESTADO_REGISTRO IN(1,59) AND B.CARRERA=A.CARRERA AND C.CURSO=A.CURSO AND D.INSTITUCION=A.INSTITUCION AND D.SECCION=A.SECCION");
            ...
            while (rs.next()) {
                ...
                item.setItemCarrera(new carreraC(rs.getString("CARRERA"), rs.getString("DESCARRERA"), rs.getString("ABRECARRERA"), null, 1));
    ...
    public List<cursoSeccionDocenteC> mostrarCursoSeccionDocenteSecundario(int institucion, String periodo,String carrera,String malla,String curso,String seccion, String personal) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                item.setItemCarrera(new carreraC(rs.getString("CARRERA"), rs.getString("DESCARRERA"), rs.getString("ABRECARRERA"), null, 1));
                item.setItemCurso(new cursoC(rs.getString("CURSO"), rs.getString("DESCURSO"), null, 1));
                item.setItemSeccion(new seccionC(rs.getInt("INSTITUCION"), rs.getString("DESSECCION"), null, 1));
                lista.add(item);
    ...
    public cursoSeccionDocenteC mostrarPeriodoCarreraCursoSeccion(int institucion, String periodo, String malla, String carrera, String curso, String seccion) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT A.*,B.DESCRIPCION DESCARRERA,C.DESCRIPCION DESCURSO,D.DESCRIPCION DESSECCION FROM SIGU.HOR_CURSO_SECCION_DOCENTE A,UPA.ACA_CARRERAS B,UPA.ACA_CURSOS C,SIGU.HOR_SECCION D WHERE A.INSTITUCION=? AND A.PERIODO=? AND A.MALLA=? AND  A.CARRERA=? AND A.CURSO=? AND  A.SECCION=? AND A.ESTADO_REGISTRO IN(1,59) AND  B.CARRERA=A.CARRERA AND C.CURSO=A.CURSO AND D.INSTITUCION=A.INSTITUCION AND D.SECCION=A.SECCION");
            cs.setInt(1, institucion);
            cs.setString(2, periodo);
