...
public String insertarCodigoDocente(int  modalidad,String periodo,String personal,String persona,String cpersonal) {
    ...
    public List<personalC> filtroPersonal(String sede) {
        Connection c ;
        CallableStatement cs ;
        ResultSet rs ;
        personalC item ;
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT A.*,B.APELLIDO_PATERNO,B.APELLIDO_MATERNO,B.NOMBRES FROM SIGU.PER_PERSONAL A,UPA.DAT_PERSONAS B where ISNULL(A.SEDE,0) LIKE ? AND B.PERSONA=A.PERSONA ORDER BY B.APELLIDO_PATERNO,B.APELLIDO_MATERNO,B.NOMBRES");
            cs.setString(1, sede);
            rs = cs.executeQuery();
            while (rs.next()) {
                item = new personalC();
