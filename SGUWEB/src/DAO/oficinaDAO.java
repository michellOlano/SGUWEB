...
public class oficinaDAO extends Conexion implements Serializable  {
	...
	public String insertar(oficinaC item) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("{CALL DI.SP_MANTENIMIENTO_OFICINA(?,?,?,?,?,?,?)}");
            cs.setInt("ITEMWEB", 1);
	...
	public List<oficinaC> listaOficinas(int institucion) {
        ...
        try {
            ...
            c = ConexionWeb();
            cs = c.prepareCall("SELECT *from SIGU.PER_OFICINA WHERE INSTITUCION=?   ORDER BY DESCRIPCION");
            cs.setInt(1, institucion);
            rs = cs.executeQuery();
