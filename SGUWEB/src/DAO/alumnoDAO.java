...
public class alumnoDAO extends Conexion implements Serializable {
 ...
 public String eliminar(alumnoC item) {
    ...
    public List<alumnoC> listarAlumno(String persona) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT *FROM SIGU.ACA_ALUMNO WHERE PERSONA=? ");
            cs.setString(1, persona);            
            rs = cs.executeQuery();
            while (rs.next()) {
                item = new alumnoC();
    ...
    public List<alumnoC> listarAlumno(int institucion,String persona) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT *FROM SIGU.ACA_ALUMNO WHERE INSTITUCION=? AND PERSONA=? ");
