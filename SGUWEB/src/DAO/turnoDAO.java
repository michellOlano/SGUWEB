...
public class turnoDAO implements Serializable {
	...
	public List<turnoC> listarTurno(int institucion,String periodo,String carrera,int nivelModular) {
        ...
        try {
            ...
            c = ConexionWeb();
            cs = c.prepareCall("SELECT DISTINCT B.* FROM SIGU.HOR_PERIODO_SECCION A,UPA.ACA_TURNOS B WHERE A.INSTITUCION=? and A.PERIODO=? and A.CARRERA=? AND A.NIVEL_MODULAR LIKE ? AND B.TURNO=A.TURNO");
            cs.setInt(1, institucion);
            cs.setString(2, periodo);
            cs.setString(3, carrera);
            cs.setInt(4, nivelModular);
            rs = cs.executeQuery();
            while (rs.next()) {
                item = new turnoC();
