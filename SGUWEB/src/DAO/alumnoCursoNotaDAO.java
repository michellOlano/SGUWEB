...
public class alumnoCursoNotaDAO implements Serializable {
    ...
    public List<alumnoCursoNotaC> listarAlumnoCursoNota(int institucion, String alumno) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT A.*,C.DESCRIPCION DESCURSO FROM SIGU.ACA_ALUMNO_CURSO_NOTA A,DI.HOR_FORMULA_REPROGRAMACION b,UPA.ACA_CURSOS C WHERE A.INSTITUCION=? AND  A.ALUMNO=? AND B.INSTITUCION=A.INSTITUCION AND B.PERIODO=A.PERIODO AND B.TIPO_EXAMEN=A.TIPO_EXAMEN AND A.NOTA BETWEEN B.MIN_NOTA AND B.MAX_NOTA AND C.CURSO=A.CURSO");
            cs.setInt(1, institucion);
