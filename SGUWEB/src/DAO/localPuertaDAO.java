...
public class localPuertaDAO {
    public localPuertaC mostrarLocalPuerta(String estacion,String cookies) {
        ...
        try {
            ...
            c = ConexionWeb();
            cs = c.prepareCall("select *from SIGU.LOG_LOCAL_PUERTA WHERE (ESTACION=? OR COOKIES=?)  AND ESTADO_REGISTRO=1");
            cs.setString(1, estacion);
            cs.setString(2, cookies);
            rs = cs.executeQuery();
