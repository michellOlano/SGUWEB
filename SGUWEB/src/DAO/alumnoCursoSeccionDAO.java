...
public class alumnoCursoSeccionDAO implements Serializable {
    ...
    public List<alumnoCursoSeccionC> mostrarAlumnoCursosSeccionMatriculados(int institucion, String periodo, String alumno) {
        ...
        try {
            ...
            c = ConexionWeb();
            cs = c.prepareCall("SELECT A.*,B.DESCRIPCION DESCARRERA,C.DESCRIPCION DESCURSO,D.DESCRIPCION DESSECCION FROM SIGU.ACA_ALUMNO_CURSO_SECCION A,UPA.ACA_CARRERAS B,UPA.ACA_CURSOS C,SIGU.HOR_SECCION D WHERE A.INSTITUCION=? AND A.PERIODO=? AND A.ALUMNO=?  AND A.ESTADO_REGISTRO IN(1,10) AND B.CARRERA=A.CARRERA AND C.CURSO=A.CURSO AND D.SECCION=A.SECCION");
            cs.setInt(1, institucion);
            cs.setString(2, periodo);
            cs.setString(3, alumno);
    ...
    public List<alumnoCursoSeccionC> mostrarAlumnoCursoSeccion(int institucion, String periodo, String carrera, String malla, String curso, String seccion,String alumno) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                item.setItemCarrera(new carreraC(rs.getString("CARRERA"), rs.getString("DESCARRERA"), null, rs.getString("COLOR"), 1));
