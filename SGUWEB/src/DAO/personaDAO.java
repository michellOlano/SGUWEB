...
public personaC accesoBolsaTrabajo(String usuario,String clave) {
    ...
    try {
        c = ConexionWeb();
        cs = c.prepareCall("SELECT B.* FROM DI.TRAB_USUARIO A, UPA.DAT_PERSONAS B WHERE B.PERSONA=A.PERSONA AND USUARIO =? AND CLAVE=? ");
...
public empresaTrabC accesoBolsaTrabajoEmpresa(String usuario,String clave) {
	
    Connection c ;
    CallableStatement cs  ;
    ResultSet rs ;
    
    empresaTrabC item =null;
    try {
        c = ConexionWeb();
        cs = c.prepareCall("SELECT A.* FROM DI.TRAB_EMPRESA A,DI.TRAB_USUARIO_EMPRESA B WHERE B.EMPRESA=A.EMPRESA AND B.USUARIO=? AND B.CLAVE=?  ");
        cs.setString(1, usuario);
        cs.setString(2, clave);
...
public List<personaC> listaEmpresaPersona(String empresa) {
    ...
    try {
        c = ConexionWeb();
        cs = c.prepareCall("SELECT B.* FROM DI.DAT_EMPRESA_PERSONA A,UPA.DAT_PERSONAS B WHERE A.EMPRESA=? AND B.PERSONA=A.PERSONA ORDER BY APELLIDO_PATERNO,APELLIDO_MATERNO,NOMBRES");
...
public personaC mostrarPersonaDNI(String numeroDocumento) {
    ...
    try {
        c = ConexionWeb();
        cs = c.prepareCall("SELECT *FROM UPA.DAT_PERSONAS WHERE NRO_DOCUMENTO=? ");
	...
	public personaC validaDNI(String persona,String numero) {
	 ...
	 public String insertar(personaC item) {
	        ...
	        try {             
	            ...
	            cs.setDouble("UBIGEO_LATITUD", item.getUbigeoLatitud());
	            cs.setDouble("UBIGEO_LONGITUD", item.getUbigeoLongitud());
	            ...
	            cs.setDouble("DIRECCION_LATITUD", item.getDireccionLatitud());
	            cs.setDouble("DIRECCION_LONGITUD", item.getDireccionLongitud());	            
	            ...
	            cs.registerOutParameter("PERSONA", Types.VARCHAR);
	            rpta = cs.executeUpdate() == 1 ;
	            if (rpta) {

	                c.commit();
	                codigo = cs.getString("PERSONA");
    ...
    public List<personaC> filtroPersona(String paterno, String materno, String nombre) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                item.setNacimiento_ubigeo(rs.getString("NACIMIENTO_UBIGEO")==null?"000000":rs.getString("NACIMIENTO_UBIGEO"));
                item.setDireccion_ubigeo(rs.getString("DIRECCION_UBIGEO")==null?"000000":rs.getString("DIRECCION_UBIGEO"));
                item.setUbigeoLatitud(rs.getDouble("UBIGEO_LATITUD"));
                item.setUbigeoLongitud(rs.getDouble("UBIGEO_LONGITUD"));
                item.setDireccion_recidencial(rs.getString("DIRECCION_RECIDENCIA"));
                item.setDireccion_referencial(rs.getString("DIRECCION_REFERENCIA"));
                item.setDireccionLatitud( rs.getDouble("DIRECCION_LATITUD"));
                item.setDireccionLongitud(rs.getDouble("DIRECCION_LONGITUD"));
                item.setEstado_civil(rs.getInt("ESTADO_CIVIL"));
                item.setTelefono(rs.getString("TELEFONO"));
                item.setCelular(rs.getString("telefono_celular"));
                item.setEmailP(rs.getString("email_principal"));
...
public personaC mostrarPersonaJurado(int institucion,String periodo,String dni) {
    ...
    public boolean validaNumeroDocumento(String numero){
        Connection c ;
        CallableStatement cs ;
        ResultSet rs ;
        boolean bandera = false;
    ...
    public boolean validaCorreo(String correo){
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT *FROM UPA.DAT_PERSONAS WHERE EMAIL_PRINCIPAL=?");            
...
public List<personaC> listaPersonaCaja() {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall("SELECT B.*FROM SIGU.PER_PERSONAL A,UPA.DAT_PERSONAS B WHERE B.PERSONA=A.PERSONA AND A.PERSONAL IN(SELECT PERSONAL FROM DI.VISTA_PERSONA_CAJA) ORDER BY 2,3,4");
...
public personaC mostrarUltimaPersona() {
    ...
    try {
        ...
        c = ConexionWeb();
        cs = c.prepareCall("SELECT TOP 1 * FROM UPA.DAT_PERSONAS ORDER BY CONTROL_FECHA DESC");
